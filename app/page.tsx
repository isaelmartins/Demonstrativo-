...
export default function Home() {
  ...
  const parseXML = (xmlText: string, fund: 'Federal' | 'Estadual') => {
    ...
    if (detTags.length === 0) {
      ...
      const qCom = xmlDoc.getElementsByTagName("qCom")[0]?.textContent || "1";
    ...
    const newPayments = detTags.map((det) => {
      ...
      const qCom = det.getElementsByTagName("qCom")[0]?.textContent || "1";
        ...
        <main className="flex-1 ml-0 md:ml-64 p-4 md:p-8 lg:p-12 pb-24 pt-28 print:m-0 print:p-0 print:ml-0 print:w-[297mm]">
          ...
          <div className="max-w-[1122px] mx-auto mb-8 no-print">
            <div className="bg-surface-container-low p-4 md:p-6 rounded-2xl border border-outline-variant/30 flex flex-col lg:flex-row items-center justify-between gap-6">
              ...
              <div className="flex flex-wrap gap-3 w-full lg:w-auto justify-center md:justify-end">
                <label className={`flex-1 sm:flex-none bg-primary text-white py-3 px-6 rounded-xl font-bold text-sm flex items-center justify-center gap-2 shadow-md hover:shadow-lg active:scale-95 transition-all cursor-pointer ${isUploading ? 'opacity-50' : ''}`}>
                ...
                <label className={`flex-1 sm:flex-none bg-tertiary-fixed-dim text-on-tertiary-fixed-variant py-3 px-6 rounded-xl font-bold text-sm flex items-center justify-center gap-2 shadow-md hover:shadow-lg active:scale-95 transition-all cursor-pointer ${isUploading ? 'opacity-50' : ''}`}>
                ...
                <button 
                  ...
                  className="flex-1 sm:flex-none bg-secondary-container text-on-secondary-container py-3 px-6 rounded-xl font-bold text-sm flex items-center justify-center gap-2 shadow-md hover:bg-secondary hover:text-white active:scale-95 transition-all cursor-pointer border border-secondary/20"
                >
                  <Settings size={16} />
                ...
                <button 
                  ...
                  className="flex-1 sm:flex-none bg-tertiary-container text-on-tertiary-container py-3 px-6 rounded-xl font-bold text-sm flex items-center justify-center gap-2 shadow-md hover:bg-tertiary hover:text-white active:scale-95 transition-all cursor-pointer border border-tertiary/20"
                >
                  <Calendar size={16} />
                  <span>Config. Relatório</span>
                </button>
                <button 
                  onClick={() => setShowClearConfirm(true)}
                  className="flex-1 sm:flex-none bg-error-container text-on-error-container py-3 px-6 rounded-xl font-bold text-sm flex items-center justify-center gap-2 shadow-md hover:bg-error hover:text-white active:scale-95 transition-all cursor-pointer border border-error/20"
                >
                  <Trash2 size={16} />
          ...
          <div className="overflow-x-auto pb-12 -mx-4 px-4 md:mx-0 md:px-0 custom-scrollbar print:overflow-visible print:m-0 print:p-0 print:block">
            <div className="min-w-[1122px] mx-auto space-y-8 print:min-w-0 print:w-full print:m-0 print:block">
              {(filterFund === 'All' ? ['Federal', 'Estadual'] : [filterFund]).map((fund) => {
                const fundPayments = payments.filter(p => p.fund === fund);
                const fundTotal = fundPayments.reduce((acc, p) => acc + p.total, 0);
                
                // Only show the sheet if there are payments or if specifically filtered
                if (fundPayments.length === 0 && filterFund === 'All') return null;

                return (
                  <div key={fund} className="print-container bg-white text-black shadow-xl p-8 w-[1122px] min-h-[794px] flex flex-col border border-black font-serif relative print:shadow-none print:border-none print:m-0 print:p-0 print:w-[297mm] print:h-[210mm]">
                    ...
                    {/* Document Header */}
                  <div className="flex justify-between items-start border-b border-black pb-2 mb-1">
                    <div className="flex items-center gap-4">
                      <div className="w-20 h-20 border border-black rounded-full flex flex-col items-center justify-center text-[9px] font-bold text-center leading-tight p-1">
                        <span>PNAE</span>
                        <span>MERENDA</span>
                        <span>ESCOLAR</span>
                      </div>
                      <div className="text-left">
                        <p className="text-[11px] font-bold uppercase leading-tight">SECRETARIA ESTADUAL DE EDUCAÇÃO - SEDUC</p>
                        <p className="text-[11px] font-bold uppercase leading-tight">FUNDO NACIONAL DE DESENVOLVIMENTO DA EDUCAÇÃO - FNDE</p>
                        <p className="text-[11px] font-bold uppercase leading-tight">PROGRAMA NACIONAL DE ALIMENTAÇÃO ESCOLAR - PNAE</p>
                      </div>
                    </div>
                    <div className="text-right flex flex-col justify-between h-20">
                      <p className="text-[12px] font-bold uppercase">PRESTAÇÃO DE CONTAS - {fund.toUpperCase()}</p>
                      <p className="text-[9px] leading-tight font-bold">Demonstrativos da execução da receita e da despesa e relação</p>
                      <p className="text-[9px] leading-tight font-bold">de pagamentos</p>
                    </div>
                  </div>
                  ...
                  <div className="mb-2">
                    ...
                    <div className="grid grid-cols-12 border-t border-l border-black">
                      ...
                      <div className="col-span-6 border-r border-b border-black p-1">
                        <label className="block text-[7px] font-bold">04 – NOME DA(S) ESCOLA(S) BENEFICIADA(S)</label>
                        <p className="text-[9px] font-bold uppercase">{schoolInfo.name}</p>
                      </div>
                      <div className="col-span-1 border-r border-b border-black p-1">
                        <label className="block text-[7px] font-bold">05 – DEP.ADM.</label>
                        <p className="text-[8px] font-bold uppercase">{fund.toUpperCase()}</p>
                      </div>
                  ...
                  <div className="mb-2">
                    ...
                    <div className="grid grid-cols-10 border-t border-l border-black text-center">
                      <div className="border-r border-b border-black p-1">
                        ...
                        <p className="text-[8px] font-bold">R$ {formatNumber(fundTotal)}</p>
                      </div>
                      <div className="border-r border-b border-black p-1">
                      ...
                      <div className="col-span-2 border-r border-b border-black p-1">
                        <label className="block text-[6px] font-bold leading-tight uppercase">14–PERÍODO DE EXECUÇÃO</label>
                      ...
                      <div className="col-span-2 border-r border-b border-black p-1">
                        ...
                        <p className="text-[8px] font-bold uppercase">{reportConfig.installment}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 space-y-4">
                    ...
                    {(() => {
                      ...
                      return invoiceEntries.map(([docNum, invoicePayments], invoiceIdx) => {
                        const invoiceTotal = invoicePayments.reduce((acc, p) => acc + p.total, 0);
                        const firstPay = invoicePayments[0];
                        
                        return (
                          <div key={docNum} className="border-t border-l border-black mb-4 last:mb-0">
                            <table className="w-full border-collapse text-[7px]">
                              <thead>
                                <tr className="text-center font-bold uppercase text-[8px]">
                                  ...
                                  <th rowSpan={2} className="border-r border-b border-black p-1 w-16 text-[8px] font-bold">21 – QUANTIDADE</th>
                                  <th rowSpan={2} className="border-r border-b border-black p-1 w-12 text-[8px] font-bold">UNIDADE</th>
                                ...
                                <tr className="text-center font-bold uppercase text-[7px]">
                                  ...
                                  <th className="border-r border-b border-black p-1 w-16">
                                    <div className="flex flex-col items-center justify-center leading-tight">
                                      ...
                                      <span className="text-[10px]">{firstPay?.doc.includes(' ') ? firstPay?.doc.split(' ')[1] : firstPay?.doc}</span>
                                    </div>
                                  </th>
                              ...
                              <tbody>
                                {invoicePayments.map((payment) => {
                                  const globalIdx = fundPayments.indexOf(payment);
                                  return (
                                    <tr key={payment.id} className="text-center">
                                      <td className="border-r border-b border-black p-1 text-[11px] font-bold">{globalIdx + 1}</td>
                                      ...
                                      <td className="border-r border-b border-black p-1 text-center uppercase text-[11px] font-bold">
                                        {payment.qty.split(' ')[1]}
                                      </td>
                                      <td className="border-r border-b border-black p-1 text-center font-bold text-[11px]">
                                ...
                                {invoiceIdx === invoiceEntries.length - 1 && 
                                 Array.from({ length: Math.max(0, 15 - fundPayments.length) }).map((_, i) => (
                                  <tr key={`empty-${i}`} className="h-8">
                                    <td className="border-r border-b border-black p-1 text-[11px] font-bold">{fundPayments.length + i + 1}</td>
                                    <td className="border-r border-b border-black p-1"></td>
                                    <td className="border-r border-b border-black p-1"></td>
                                    <td className="border-r border-b border-black p-1"></td>
                                    <td className="border-r border-b border-black p-1"></td>
                              ...
                              <tfoot>
                                <tr className="font-bold uppercase text-[9px]">
                                  <td className="border-r border-b border-black p-1"></td>
                                  <td className="border-r border-b border-black p-1 text-left">TOTAL DA NOTA FISCAL {docNum}</td>
                                ...
                                {invoiceIdx === invoiceEntries.length - 1 && (
                                  <tr className="font-bold uppercase text-[10px] bg-gray-50">
                                    <td className="border-r border-b border-black p-1"></td>
                                    <td className="border-r border-b border-black p-1 text-left">TOTAL GERAL DO REPASSE {fund.toUpperCase()}</td>
                                    <td className="border-r border-b border-black"></td>
                                    <td className="border-r border-b border-black"></td>
                                    <td className="border-r border-b border-black"></td>
                                    <td className="border-r border-b border-black"></td>
                                    ...
                                    <td className="border-r border-b border-black"></td>
                                    <td className="border-r border-b border-black"></td>
                                    <td className="border-b border-black p-1 text-center text-[12px]">{formatNumber(fundTotal)}</td>
                                  </tr>
                                )}
                              </tfoot>
                            </table>

                            {/* Signature Section - Only on the last invoice table of the fund */}
                            {invoiceIdx === invoiceEntries.length - 1 && (
                              <div className="mt-4">
                                <p className="text-[7px] font-bold mb-2">OBS. ACOMPANHA ESTA PRESTAÇÃO DE CONTAS, OFÍCIO DE ENCAMINHAMENTO DA PRESTAÇÃO DE CONTAS, EXTRATO BANCÁRIO, PARECER DO CONSELHO E COMPROVANTES DE DESPESAS.</p>
                                <div className="grid grid-cols-2 border border-black">
                                  <div className="border-r border-black p-2 flex flex-col justify-between">
                                    <div className="flex gap-1 items-baseline">
                                      <span className="text-[8px] font-bold">23 – AUTENTICAÇÃO:</span>
                                      <span className="text-[9px] font-bold underline">{reportConfig.authDate}</span>
                                    </div>
                                    <span className="text-[7px] font-bold text-center mt-1">Local e Data</span>
                                  </div>
                                  <div className="p-2 flex flex-col items-center justify-end">
                                    <div className="border-t border-black w-full mb-1"></div>
                                    <span className="text-[7px] font-bold uppercase">Assinatura do Gestor Responsável</span>
                                  </div>
                                </div>
                              </div>
                            )}
        ...
        {selectedPayment && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.9, opacity: 0, y: 20 }}
              className="bg-surface w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col"
              onClick={(e) => e.stopPropagation()}
            >
              <div className={`p-6 text-white flex justify-between items-start ${selectedPayment.fund === 'Federal' ? 'bg-primary' : 'bg-tertiary-fixed-dim'}`}>
                <div>
                  <h3 className="font-headline font-black text-2xl mb-1">{selectedPayment.name}</h3>
                  <p className="text-sm opacity-80 font-medium">CNPJ: {selectedPayment.cnpj}</p>
                </div>
              ...
              <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <h4 className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Detalhes do Item</h4>
                  <p className="font-medium text-lg mb-4">{selectedPayment.item}</p>
                  
                  <h4 className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Quantidade</h4>
                  <p className="font-medium text-lg mb-4">{selectedPayment.qty}</p>
                  
                  <h4 className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Fundo</h4>
                  <span className={`inline-block px-3 py-1 text-xs font-bold rounded-full uppercase tracking-tighter ${selectedPayment.fund === 'Federal' ? 'bg-primary/10 text-primary' : 'bg-tertiary-fixed/30 text-tertiary-fixed-dim'}`}>
                    {selectedPayment.fund}
                  </span>
                </div>
                <div className="bg-surface-container-low p-6 rounded-xl border border-outline-variant/20">
                  <div className="mb-4">
                    <h4 className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Documento Fiscal</h4>
                    <p className="font-medium">{selectedPayment.doc} <span className="text-xs opacity-60 ml-2">{selectedPayment.docDate}</span></p>
                  </div>
                  <div className="mb-6">
                    <h4 className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Ordem de Pagamento</h4>
                    <p className="font-medium">{selectedPayment.payDoc} <span className="text-xs opacity-60 ml-2">{selectedPayment.payDate}</span></p>
                  </div>
                  <div className="pt-4 border-t border-outline-variant/20">
                    <h4 className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Valor Total</h4>
                    <p className="font-headline font-black text-3xl text-primary">{formatCurrency(selectedPayment.total)}</p>
                  </div>
                </div>
              </div>
              <div className="bg-surface-container-lowest p-6 border-t border-outline-variant/10 flex justify-end">
                <button onClick={() => setSelectedPayment(null)} className="px-6 py-2 bg-surface-container hover:bg-surface-variant text-on-surface font-bold rounded-lg transition-colors">
                  Fechar
                </button>
              </div>
            </motion.div>
